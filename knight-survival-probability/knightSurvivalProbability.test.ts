...
import { knightSurvivalProbability } from "./knightSurvivalProbability";
...
describe("knightSurvivalProbability", () => {
  it("example: 3x3, one move from a corner", () => {
    close(knightSurvivalProbability(3, 1, 0, 0), 0.25);
  });

  it("k = 0 is always 1", () => {
    close(knightSurvivalProbability(1, 0, 0, 0), 1);
    close(knightSurvivalProbability(8, 0, 3, 4), 1);
  });

  it("1x1, one move leaves the board", () => {
    close(knightSurvivalProbability(1, 1, 0, 0), 0);
  });

  it("2x2, one move always leaves", () => {
    close(knightSurvivalProbability(2, 1, 0, 0), 0);
  });
  ...
  describe("naive cross-check (small k)", () => {
    ...
    for (const [n, k, r, c] of cases) {
      it(`n=${n} k=${k} (${r},${c})`, () => {
        close(knightSurvivalProbability(n, k, r, c), naive(n, k, r, c));
      });
    }
  });
});
