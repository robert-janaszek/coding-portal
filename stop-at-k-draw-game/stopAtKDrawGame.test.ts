...
import { stopAtKDrawGame } from "./stopAtKDrawGame";
...
describe("stopAtKDrawGame", () => {
  it("example: one draw always <= n", () => {
    close(stopAtKDrawGame(8, 1, 8), 1);
  });

  it("example: one draw, 6 of 10 succeed", () => {
    close(stopAtKDrawGame(4, 1, 8), 0.5);
  });

  it("example: stop window sits under n", () => {
    close(stopAtKDrawGame(12, 8, 4), 1);
  });

  it("k = 0 never draws", () => {
    close(stopAtKDrawGame(0, 0, 1), 1);
    close(stopAtKDrawGame(10, 0, 10), 1);
  });

  it("cannot overshoot when n covers the whole window", () => {
    close(stopAtKDrawGame(10, 1, 10), 1);
    close(stopAtKDrawGame(20, 10, 5), 1);
  });
  ...
  describe("oracle cross-check (small)", () => {
    ...
    for (const [n, k, maxPts] of cases) {
      it(`n=${n} k=${k} maxPts=${maxPts}`, () => {
        close(stopAtKDrawGame(n, k, maxPts), oracle(n, k, maxPts));
      });
    }
  });
});
