...
import { ListNode, cycleInLinkedList } from "./cycleInLinkedList";
...
describe("cycleInLinkedList", () => {
  it("empty list", () => {
    assert.equal(cycleInLinkedList(null), false);
  });

  it("no cycle", () => {
    const head = buildList([7], -1);
    assert.equal(cycleInLinkedList(head), false);
  });

  it("cycle starting at index 2", () => {
    const head = buildList([5, 1, 9], 2);
    assert.equal(cycleInLinkedList(head), true);
  });
