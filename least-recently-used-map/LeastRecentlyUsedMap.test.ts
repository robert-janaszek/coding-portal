...
import { LeastRecentlyUsedMap } from "./LeastRecentlyUsedMap";

describe("LeastRecentlyUsedMap", () => {
  it("example", () => {
    const cache = new LeastRecentlyUsedMap(2);
    cache.put(10, 10);
    cache.put(20, 20);
  ...
  it("capacity 1", () => {
    const cache = new LeastRecentlyUsedMap(1);
    cache.put(1, 1);
  ...
  it("overwrite with spare capacity keeps other keys", () => {
    const cache = new LeastRecentlyUsedMap(3);
    cache.put(1, 1);
    cache.put(2, 2);
    cache.put(1, 10);
    cache.put(3, 3);
