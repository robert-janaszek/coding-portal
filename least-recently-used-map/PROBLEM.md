...
- `LeastRecentlyUsedMap(capacity)` — empty store, `capacity >= 1`
...
| File | Role |
|------|------|
| `LeastRecentlyUsedMap.ts` | Stub class |
| `LeastRecentlyUsedMap.test.ts` | Tests |
| `HINTS.md` | Spoilers — open only if stuck |
| `solutions/` | Archived attempts (spoilers) |

```bash
npm run test:least-recently-used-map
```
