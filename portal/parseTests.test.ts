...
describe("parseTestsFromSource", () => {
    ...
    assert.deepEqual(tests, [
      {
        ...
        fullName: "knightProbability k = 0 is always 1",
        suites: ["knightProbability"],
      },
    ]);
  });
  ...
  it("uses enclosing describe as fullName for interpolated it titles", () => {
    ...
    assert.equal(tests[0]!.fullName, "knightProbability naive cross-check (small k)");
...
✔ knightProbability (1.5ms)
`;
    const mapped = statusesFromSpecResults(tests, parseSpecResults(spec));
    assert.equal(mapped.get("knightProbability naive cross-check (small k)"), "pass");
  });
});
