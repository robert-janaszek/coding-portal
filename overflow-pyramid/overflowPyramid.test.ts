...
import { overflowPyramid } from "./overflowPyramid";
...
describe("overflowPyramid", () => {
  it("example: 1 cup, query row 1 is empty", () => {
    close(overflowPyramid(1, 1, 0), 0);
  });

  it("example: 2 cups, row 1 is half full", () => {
    close(overflowPyramid(3, 1, 0), 1);
  });

  it("example: huge pour fills (33, 17)", () => {
    close(overflowPyramid(8, 2, 1), 1);
  });

  it("poured = 0 is always empty", () => {
    close(overflowPyramid(0, 0, 0), 0);
    close(overflowPyramid(0, 5, 2), 0);
  });

  it("top glass saturates at 1", () => {
    close(overflowPyramid(1, 0, 0), 1);
    close(overflowPyramid(50, 0, 0), 1);
  });

  it("3 cups fill row 1 and leave row 2 empty", () => {
    close(overflowPyramid(3, 1, 0), 1);
    close(overflowPyramid(3, 1, 1), 1);
    close(overflowPyramid(3, 2, 0), 0);
    close(overflowPyramid(3, 2, 1), 0);
    close(overflowPyramid(3, 2, 2), 0);
  });
  ...
  describe("oracle cross-check (small pyramids)", () => {
    ...
    for (const [poured, row, glass] of cases) {
      it(`poured=${poured} (${row},${glass})`, () => {
        close(overflowPyramid(poured, row, glass), oracle(poured, row, glass));
      });
    }
  });
});
