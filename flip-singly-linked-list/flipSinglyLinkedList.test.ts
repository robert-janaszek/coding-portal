...
import { ListNode, flipSinglyLinkedList } from "./flipSinglyLinkedList";
...
describe("flipSinglyLinkedList", () => {
  it("empty list", () => {
    assert.equal(flipSinglyLinkedList(null), null);
  });

  it("single node", () => {
    const head = new ListNode(1);
    const reversed = flipSinglyLinkedList(head);
    assert.equal(reversed, head);
    assert.deepEqual(toArray(reversed), [1]);
  });

  it("example 1", () => {
    const reversed = flipSinglyLinkedList(buildList([8, 3, 1, 6]));
    assert.deepEqual(toArray(reversed), [6, 1, 3, 8]);
  });

  it("example 2", () => {
    const reversed = flipSinglyLinkedList(buildList([4, 9]));
